...
/// 检查是否已认证（有任意账号）
pub async fn copilot_is_authenticated(state: Arc<CopilotAuthState>) -> Result<bool, String> {
    let auth_manager = state.0.read().await;
    Ok(auth_manager.is_authenticated().await)
}

/// 注销所有 Copilot 认证
pub async fn copilot_logout(state: Arc<CopilotAuthState>) -> Result<(), String> {
    let auth_manager = state.0.write().await;
...
///
/// 内部使用，用于代理请求
pub async fn copilot_get_token(state: Arc<CopilotAuthState>) -> Result<String, String> {
    let auth_manager = state.0.read().await;
    auth_manager
        .get_valid_token()
        .await
