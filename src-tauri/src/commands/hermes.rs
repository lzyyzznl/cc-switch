...
pub fn import_hermes_providers_from_live(state: Arc<AppState>) -> Result<usize, String> {
    crate::services::provider::import_hermes_providers_from_live(state.as_ref())
...
pub async fn open_hermes_web_ui(path: Option<String>) -> Result<(), String> {
    ...
    // Server mode: return the URL for the frontend to open
    log::info!("Hermes Web UI available at: {target}");
    Ok(())
}
